...
if (!products) {
    products = [
        {id:1,name:"Floral Print Summer Dress",category:"dresses",price:899,cost:280,rating:4.6,reviews:124,emoji:"👗",color:"#ec4899",stock:45,image:null},
        {id:2,name:"Embroidered Anarkali Kurti",category:"ethnic",price:1499,cost:480,rating:4.8,reviews:89,emoji:"🥻",color:"#a855f7",stock:30,image:null},
        {id:3,name:"Silk Blend Saree",category:"ethnic",price:2199,cost:720,rating:4.9,reviews:201,emoji:"🥻",color:"#f59e0b",stock:25,image:null},
        {id:4,name:"Ruffle Sleeve Top",category:"tops",price:549,cost:160,rating:4.4,reviews:156,emoji:"👚",color:"#06b6d4",stock:70,image:null},
        {id:5,name:"Tailored Palazzo Pants",category:"bottoms",price:699,cost:210,rating:4.5,reviews:98,emoji:"👖",color:"#10b981",stock:55,image:null},
        {id:6,name:"Pearl Drop Earrings",category:"accessories",price:299,cost:60,rating:4.7,reviews:312,emoji:"💎",color:"#f43f5e",stock:120,image:null},
        {id:7,name:"Designer Clutch Purse",category:"accessories",price:799,cost:200,rating:4.6,reviews:74,emoji:"👜",color:"#8b5cf6",stock:40,image:null},
        {id:8,name:"Lace Midi Dress",category:"dresses",price:1299,cost:420,rating:4.7,reviews:67,emoji:"👗",color:"#ec4899",stock:20,image:null},
        {id:9,name:"Cotton Kantha Kurti",category:"ethnic",price:799,cost:240,rating:4.5,reviews:183,emoji:"👚",color:"#a855f7",stock:60,image:null},
        {id:10,name:"Off-Shoulder Blouse Top",category:"tops",price:649,cost:190,rating:4.3,reviews:92,emoji:"👚",color:"#06b6d4",stock:48,image:null}
    ];
    ...
    grid.innerHTML = filtered.length ? filtered.map(p => `
        <div class="product-card" style="--product-color:${p.color}" data-id="${p.id}">
            ...
            <div class="product-image">
                ...
                ${p.image ? `<img src="${p.image}" alt="${p.name}">` : `<span class="product-emoji">${p.emoji}</span>`}
            </div>
...
function renderCart() {
    ...
    items.innerHTML = cart.map(item => {
        ...
        return `
            <div class="cart-item">
                <div class="cart-item-emoji">${p.image ? `<img src="${p.image}" alt="${p.name}">` : p.emoji}</div>
...
function placeOrder() {
    ...
    const orderItems = cart.map(item => {
        const p = products.find(x => x.id === item.id);
        return { id: item.id, name: p?.name || '', emoji: p?.emoji || '', price: p?.price || 0, qty: item.qty, cost: p?.cost || 0, image: p?.image || null };
    });
...
function renderAdminProducts() {
    ...
    grid.innerHTML = products.map(p => `
        <div class="admin-product-card">
            <div class="admin-product-emoji">${p.image ? `<img src="${p.image}" alt="${p.name}">` : p.emoji}</div>
